...
def generate_random_opcode(num_opcodes, file_path):
    ...
    opcodes = ['mul', 'addi']  # Only include 'mul' and 'addi' for subsequent lines
    
    with open(file_path, 'w') as file:
        # Generate the first opcode line with 'ld'
        address = f"{random.randint(0x40000000, 0x4FFFFFFF):08x}"
        immediate = random.randint(1, 500)  # Random immediate value
...
num_opcodes = 50  # You can change this to generate more or fewer opcodes
file_path = 'opcodes.txt'  # Output file name
